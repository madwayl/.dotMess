...
class NemoTerminal(object):
    ...
    def __init__(self, uri, window):
        ...
        self.term.set_font(Pango.FontDescription.from_string("FantasqueSansM Nerd Font Propo, 11"))
        ...
        palette = [
            # 0 - Black (dark base for terminal text)
            Gdk.RGBA(0.11, 0.12, 0.14, 1.0),  # Dark background color (blackish)

            # 1 - Red
            Gdk.RGBA(0.86, 0.18, 0.23, 1.0),  # Gruvbox Red

            # 2 - Green
            Gdk.RGBA(0.18, 0.78, 0.29, 1.0),  # Gruvbox Green

            # 3 - Yellow
            Gdk.RGBA(0.88, 0.73, 0.27, 1.0),  # Gruvbox Yellow

            # 4 - Blue
            Gdk.RGBA(0.32, 0.42, 0.73, 1.0),  # Gruvbox Blue

            # 5 - Purple
            Gdk.RGBA(0.76, 0.47, 0.62, 1.0),  # Gruvbox Purple

            # 6 - Aqua
            Gdk.RGBA(0.62, 0.61, 0.45, 1.0),  # Gruvbox Aqua

            # 7 - Light Gray (White in darker themes)
            Gdk.RGBA(0.89, 0.79, 0.58, 1.0),  # Gruvbox Light Gray

            # 8 - Dark Gray
            Gdk.RGBA(0.36, 0.33, 0.31, 1.0),  # Dark Gray (dimmed background or text)

            # 9 - Red 2 (A second shade of red)
            Gdk.RGBA(0.76, 0.22, 0.29, 1.0),  # Gruvbox Red 2

            # 10 - Green 2 (A second shade of green)
            Gdk.RGBA(0.38, 0.60, 0.23, 1.0),  # Gruvbox Green 2

            # 11 - Yellow 2
            Gdk.RGBA(0.74, 0.56, 0.34, 1.0),  # Gruvbox Yellow 2

            # 12 - Blue 2
            Gdk.RGBA(0.61, 0.42, 0.85, 1.0),  # Gruvbox Blue 2

            # 13 - Purple 2 (Lighter purple, for additional variety)
            Gdk.RGBA(0.64, 0.45, 0.91, 1.0),  # Light Purple

            # 14 - Aqua 2 (A light variant)
            Gdk.RGBA(0.82, 0.46, 0.26, 1.0),  # Orange (used for the aqua variant here)

            # 15 - White (Bright, used for high-contrast text)
            Gdk.RGBA(1.0, 1.0, 1.0, 1.0)      # Bright White
        ]

        self.term.set_colors(
                        # 15 - White
                        Gdk.RGBA(1.0, 1.0, 1.0, 1.0),      # White (for bright elements)
                        # 0 - Black (Background or Dark Base)
                        Gdk.RGBA(0.11, 0.12, 0.14, 0.5),  # Dark background, blackish                        
                        palette)
        
        # Warm Highlight (Red-orange background, light text)
        self.term.set_color_highlight(Gdk.RGBA(0.9, 0.3, 0.2, 1.0))
        self.term.set_color_highlight_foreground(Gdk.RGBA(0.9, 0.9, 0.9, 1.0))  # Light text

                # Set background color (dark)
        self.term.set_background_color(Gdk.RGBA(0.11, 0.12, 0.14, 1.0))  # Dark background

        # Set foreground color (light gray or white)
        self.term.set_foreground_color(Gdk.RGBA(0.89, 0.79, 0.58, 1.0))  # Light gray (or white)
