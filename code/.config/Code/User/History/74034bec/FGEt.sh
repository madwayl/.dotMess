...
# 4
castarea() {
	if ! pgrep -F /tmp/castarea; then
		file="Screenshot_${time}.mp4"
		wl-screenrec -g "$(slurp)" --filename=${dir}/${file} --low-power=off &
		echo "$!" > '/tmp/castarea'
	else
