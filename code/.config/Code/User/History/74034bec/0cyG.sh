...
# 4
castarea() {
	if ! pgrep $(cat /tmp/castarea) > /dev/null; then
		file="Screenshot_${time}.mp4"
		wl-screenrec -g "$(slurp)" --filename=${dir}/${file} &
		echo "$!" > '/tmp/castarea'
	else
